...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!type || !description || !location || !contact) {
      return res.status(400).json({ error: 'Missing required fields: type, description, location, contact' });
    }

    const db = getDb();

    // Run AI categorization (non-blocking on failure)
    ...
    // Only try AI matching for lost items
    let matches = [];
    if (type === 'lost') {
      try {
        const snap = await db
        ...
        // Match failure is non-fatal — item was already saved
