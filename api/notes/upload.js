...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!title?.trim() || !subject?.trim() || !uploader_name?.trim() || !raw_text?.trim()) {
      return res.status(400).json({ error: 'Missing required fields: title, subject, uploader_name, raw_text' });
    }

    // AI summarize — fallback gracefully if AI is down
